...
# Generate six markers (ids 0..5) and six corresponding MindAR target images.
for marker_id in range(6):
    png_size = 600
    png = cv2.aruco.generateImageMarker(marker_dict, marker_id, png_size)
    png_path = out_dir / f"4x4_1000-{marker_id}.png"
    ...
    svg_path = out_dir / f"4x4_1000-{marker_id}.svg"
    svg_path.write_text("\n".join(svg_lines), encoding="utf-8")

    print(f"Generated: {png_path}")
    print(f"Generated: {svg_path}")

    # Build feature-rich MindAR target image centered on the same ArUco id.
    target_size = 1400
    ...
    for y in range(ring_cells):
        for x in range(ring_cells):
            ...
            if not is_border:
                continue
            if (x + y + marker_id) % 2 == 0:
                x0 = origin + x * cell
                y0 = origin + y * cell
                cv2.rectangle(target, (x0, y0), (x0 + cell, y0 + cell), (0, 0, 0), -1)

    # Deterministic micro-features around the center (per marker id).
    rng = np.random.default_rng(42 + marker_id)
    for _ in range(120):
    ...
    mind_png_path = out_dir / f"4x4_1000-{marker_id}-mind-target.png"
