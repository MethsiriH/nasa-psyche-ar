...
// const MARKER_ID_OVERLAY_ONLY_MODE = true;
// const MARKER_ID_OVERLAY_IDS = [1, 3, 4, 6] as const;
...
//                             {MARKER_ID_OVERLAY_ONLY_MODE &&
//                                 MARKER_ID_OVERLAY_IDS.map((id) => {
//                                     const markerSize = 0.0508; // 2 inches in meters
//                                     // Inside <a-marker>, geometry uses marker-local units (marker width ~= 1).
//                                     // radius=0.5 covers the marker footprint; height tuned for clear visibility.
//                                     const radius = 0.5;
//                                     const height = 0.25;
//                                     const entry = arMarkerDebugFrame?.entries.find((m) => m.id === id);
//                                     // CENTER is global (0,0,0) in config frame.
//                                     // For a marker-local overlay, center offset is -markerPositionInConfig.
//                                     const dx = entry ? -entry.x : 0;
//                                     const dy = entry ? -entry.y : 0;
//                                     const dz = entry ? -entry.z : 0;
//                                     const centerRadius = 0.3;
//                                     return (
//                                         <a-marker key={`id-overlay-${id}`} type="barcode" value={id} size={markerSize}>
//                                             <a-cylinder
//                                                 position={`0 ${height * 0.5 + 0.02} 0`}
//                                                 radius={radius}
//                                                 height={height}
//                                                 color="#ff1f1f"
//                                                 material="shader: standard; side: double; roughness: 0.45; metalness: 0.05; opacity: 1.0; transparent: false"
//                                             />
//                                             {/* Main centroid projection logic: compute CENTER from this visible marker + config offsets. */}
//                                             <a-entity position={`${dx} ${dy + height * 0.5 + 0.02} ${dz}`}>
//                                                 <a-cylinder
//                                                     radius={centerRadius}
//                                                     height={height}
//                                                     color="#ff9f1a"
//                                                     material="shader: standard; side: double; roughness: 0.45; metalness: 0.05; opacity: 0.9; transparent: true"
//                                                 />
//                                                 <a-text
//                                                     value="CENTER"
//                                                     position={`0 ${height * 0.5 + 0.02} 0`}
...
//                                     >
//                                         {arMarkerDebugFrame.entries.map((m) => {
//                                             // Marker is 2x2 inches (0.0508m side). Use circle diameter = side*sqrt(2)
//                                             // so it fully covers the square marker footprint.
//                                             const markerSide = 1.0508;
//                                             const cylRadius = (markerSide * Math.SQRT2) / 2;
//                                             const cylHeight = 1.03;
//                                             const isTable = (AR_TABLE_BARCODE_IDS as readonly number[]).includes(m.id);
//                                             const lift = cylHeight + 0.01;
//                                             const px = m.x;
//                                             const py = m.y + lift;
//                                             const pz = m.z;
//                                             const rotStr = isTable ? '0 0 0' : rotationFromNormal(m.nx, m.ny, m.nz);
//                                             return (
//                                                 <a-entity key={`ar-marker-cylinder-${m.id}`} position={`${px} ${py} ${pz}`} rotation={rotStr}>
//                                                     <a-cylinder
//                                                         height={cylHeight}
//                                                         radius={cylRadius}
//                                                         color="#ff1f1f"
//                                                         material="shader: flat; side: double; opacity: 0.9; transparent: true; depthTest: false"
//                                                     />
//                                                 </a-entity>
//                                             );
//                                         })}
